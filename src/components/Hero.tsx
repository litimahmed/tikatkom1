...
export default function Hero({ lang, onExploreClick, onBuyFlagshipClick }: HeroProps) {
  const t = translations[lang];
  const flagshipProduct = products[0]; // Montre Intelligente T900 Ultra

  const isRTL = lang === "ar";

  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-white via-gray-50/50 to-white py-12 md:py-20 lg:py-24">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className={`lg:col-span-7 flex flex-col justify-center space-y-6 text-center items-center lg:items-start ${isRTL ? "lg:text-right" : "lg:text-left"}`}>
            ...
            <h1 className="font-display text-4xl font-extrabold tracking-tight text-brand-navy sm:text-5xl lg:text-6xl lg:leading-[1.15] w-full">
              {isRTL ? (
                <span className="text-brand-navy font-arabic font-black">
                  منتجات مختارة <br />
            ...
            <p className="text-base leading-relaxed text-gray-600 sm:text-lg max-w-2xl">
              {t.heroSubtitle}
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-2 w-full">
              ...
              <button
                ...
                className="w-full sm:w-auto rounded-xl border border-gray-200 bg-white px-8 py-4 text-base font-bold text-gray-800 transition-all duration-200 hover:bg-gray-50 hover:border-gray-300 hover:-translate-y-0.5 active:scale-95 cursor-pointer"
                id="hero-cta-buy"
              >
            ...
            {isRTL ? (
              <div className="flex flex-wrap items-center justify-center lg:justify-start gap-x-6 gap-y-2 text-sm font-arabic font-bold text-gray-600 pt-4">
                <span>الدفع عند الاستلام</span>
                <span className="h-1.5 w-1.5 rounded-full bg-brand-green/75"></span>
                <span>التوصيل لـ 58 ولاية عبر ياليدين</span>
              </div>
            ) : (
              <div className="flex items-center justify-center lg:justify-start gap-6 pt-4 text-xs text-gray-500 font-semibold">
